    ...
    <div className="flex items-start space-x-4">
      ...
      <div className="flex w-full self-stretch flex-col">
        <div className="flex items-center justify-between">
          ...
          >
            {data.title} {/* Changed from name to title to match Product */}
          </Link>
          <Button
            variant="ghost"
            size="icon"
          ...
          <CartCounter
            ...
            onRemove={() =>
              dispatch(
                removeCartItem({
                  id: data.id,
                  attributes: data.attributes.map((attr) => attr.options[0]), // Extract first option
                })
              )
            }
            isZeroDelete
